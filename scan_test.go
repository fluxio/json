...
func TestArrayScanner(t *testing.T) {
	...
	if !as.Scan() {
		t.Errorf("expected as.Scan() = true")
	}
	if s.Kind() != Number || string(s.Value()) != "1" {
		t.Errorf("expected 1")
	}
	if as.Scan() {
		t.Errorf("expected as.Scan() = false")
	}
}

func TestObjectScanner(t *testing.T) {
	s := NewScanner(strings.NewReader(`{"a": {"b":2}, "c": 1}`))
	if !s.Scan() {
		t.Errorf("expected s.Scan() = true")
	}
	if s.Kind() != Object {
	...
	if !os.Scan() {
		t.Errorf("expected os.Scan() = true")
	}
	if s.Kind() != Number || string(s.Value()) != "1" {
		t.Errorf("expected 1")
	}
	if os.Scan() {
		t.Errorf("expected os.Scan() = false")
	}
}
